...
import { useLogKegiatan } from "@/stores/logKegiatan";
import {
...
function ActivityHistory() {
      ...
      cell: ({ row }) => {
        return (
          <Button
            ...
            onClick={() => handleProgressTimeline(row.original)}
          >
            Lihat Detail
          </Button>
        );
  ...
  const [idProgress, setIdProgress] = useState<string | undefined>(undefined);
  const { query: progressKegiatan } = useLogKegiatan(idProgress);
  const [rowSelected, setRowSelected] = useState<
    RiwayatPengajuanType | undefined
  >(undefined);
  ...
  const handleProgressTimeline = (values?: RiwayatPengajuanType) => {
    setIdProgress(values?.id);
    setRowSelected(values);
    openModal("ProgressTimeline");
  };
  ...
  return (
    <>
      <ProgressTimeline
        ...
        logKegiatan={progressKegiatan?.data?.data}
      />
      <Card>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="pt-2">
                          <Button
                            variant="link"
                            className="text-green-600 hover:text-green-700 p-0 h-auto text-xs"
                            onClick={() => handleProgressTimeline(activity)}
                          >
                            Lihat Detail
                          </Button>
                        </div>
