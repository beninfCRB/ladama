...
const createGridMask = (start: number, end: number): string => {
  ...
  return `linear-gradient(to bottom, transparent 0%, rgba(55, 170, 55, 0.05) ${start}%, rgba(55, 170, 55, 0.2) ${mid}%, rgba(55, 170, 55, 0.6) ${
    end - 20
  }%, rgba(55, 170, 55, 1) ${end}%)`;
};
...
const LightRay = React.memo<LightRayProps>(
  ({
    ...
    return (
      <motion.div
        className="absolute pointer-events-none -top-[5%] left-[var(--ray-left)] w-[var(--ray-width)] h-[var(--ray-height)] origin-top mix-blend-screen bg-[linear-gradient(to_bottom,rgba(55,170,55,var(--ray-opacity)),rgba(55,170,55,0))] blur-[var(--ray-blur)] translate-x-[-50%] rotate-[var(--ray-rotation)]"
        style={
          {
            "--ray-left": left,
            "--ray-width": `${width}px`,
